...
MODEL_SCAFFOLD_STATUS_PATTERN = re.compile(
    r"^\s*implementation_status\s*=\s*['\"]scaffold['\"]",
    re.MULTILINE,
)
PLUGIN_SCAFFOLD_STATUS_PATTERN = re.compile(
    r"^\s*implementationStatus\s*:\s*['\"]scaffold['\"]",
    re.MULTILINE,
)
MODEL_FORGE_MARKER = "Generated by ModelForge."
...
def delete_scaffold_asset(
    *,
    ...
    if asset.protected or asset.deletable is not True:
        raise HTTPException(
            status_code=409,
            detail={
                "message": asset.deleteDisabledReason
                or "This asset is protected and cannot be deleted through Model Forge.",
                "kind": kind,
                "key": normalized_key,
            },
        )
    ...
    for key in keys:
        ...
        items.append(
            _build_plugin_asset_item(
                ...
                key=key,
                backend_path=backend_path,
                frontend_path=frontend_fields_path,
                project_root=project_root,
...
def _build_model_asset_item(
    *,
    project_root: Path,
    model_path: Path,
    missing_locations: list[str],
) -> ScaffoldAssetItem:
    definition_path = model_path / "definition.py"
    is_scaffold = _is_model_forge_scaffold_file(
        definition_path,
        MODEL_SCAFFOLD_STATUS_PATTERN,
    )
    reason = (
        None
        if is_scaffold
        else "This model is not an active Model Forge scaffold."
    )

    return ScaffoldAssetItem(
        kind="model",
        key=model_path.name,
        locations=[_to_relative_path(project_root, model_path)],
        missingLocations=missing_locations,
        deletable=is_scaffold,
        protected=not is_scaffold,
        origin="modelForge" if is_scaffold else "repository",
        deleteDisabledReason=reason,
    )


def _build_plugin_asset_item(
    *,
    kind: ScaffoldAssetKind,
    key: str,
    backend_path: Path,
    ...
    if not backend_path.is_dir() or not frontend_path.is_dir():
        return ScaffoldAssetItem(
            kind=kind,
            key=key,
            locations=locations,
            ...
            deleteDisabledReason=(
                f"This {_asset_label(kind)} is partial and must be inspected manually."
            ),
        )

    backend_index = backend_path / "index.js"
    frontend_index = frontend_path / "index.js"
    is_scaffold = all(
        _is_model_forge_scaffold_file(path, PLUGIN_SCAFFOLD_STATUS_PATTERN)
        for path in (backend_index, frontend_index)
    )
    reason = (
        None
        if is_scaffold
        else f"This {_asset_label(kind)} is not an active Model Forge scaffold."
    )
...
def _is_model_forge_scaffold_file(path: Path, status_pattern: re.Pattern[str]) -> bool:
    source = _read_text_file(path)
    return source is not None and MODEL_FORGE_MARKER in source and bool(
        status_pattern.search(source)
    )
...
def _asset_label(kind: ScaffoldAssetKind) -> str:
    return "evaluation structure" if kind == "evaluationStructure" else "parameter structure"
