...
export const buildIssueStateSnapshot = async ({ issue, snapshotType, consensusPhase = issue.consensusPhase, criteriaWeightingConfiguration = null, session = null }) => {
  ...
  return clone({
    ...
    criteriaWeighting: { ...(snapshotType === "creation" ? { required: criteriaWeightingConfiguration?.isCriteriaWeightingRequired === true, level: issue.criteriaWeightingLevel ?? "leaf", source: criteriaWeightingConfiguration?.source ?? null, mode: criteriaWeightingConfiguration?.mode ?? null, method: criteriaWeightingConfiguration?.method ?? null, structureKey: issue.criteriaWeightsStructureKey ?? null, model: modelSnapshot(criteriaWeightingModel), apiModelKey: issue.criteriaWeightingApiModelKey ?? null, apiEndpoint: clone(issue.criteriaWeightingApiEndpoint ?? null, "criteriaWeighting.apiEndpoint"), parameters: clone(issue.criteriaWeightingParameters ?? {}, "criteriaWeighting.parameters") } : clone(creationSnapshot.state.criteriaWeighting, "creation.criteriaWeighting")), level: issue.criteriaWeightingLevel ?? "leaf", sourceWeightsByCriterionId: clone(issue.criteriaWeightingSourceWeights ?? {}, "criteriaWeighting.sourceWeights"), weightsByCriterionId: clone(issue.modelParameters?.weights ?? {}, "criteriaWeighting.weights") },
