...
describe("getFinishedIssuesPayload", () => {
  it("returns the three best alternatives ordered by canonical rank", async () => {
    ...
    await createStageResult({
      issueId: issue._id,
      rankedAlternatives: [
        { alternativeId: "alternative-5", name: "Fifth", rank: 5 },
        { alternativeId: "alternative-1", name: "First", rank: 1 },
        { alternativeId: "alternative-4", name: "Fourth", rank: 4 },
        { alternativeId: "alternative-3", name: "Third", rank: 3 },
        { alternativeId: "alternative-2", name: "Second", rank: 2 },
      ],
    });
  ...
  it("uses each issue's highest alternative-evaluation phase and never a criteria result", async () => {
    ...
    await createStageResult({
      ...
      phase: 0,
      rankedAlternatives: [{ alternativeId: "old", name: "Earlier round", rank: 1 }],
    });
    await createStageResult({
      issueId: consensusIssue._id,
      phase: 2,
      rankedAlternatives: [{ alternativeId: "final", name: "Final round", rank: 1 }],
    });
    await createStageResult({
      issueId: consensusIssue._id,
      stage: "criteriaWeighting",
      rankedAlternatives: [{ alternativeId: "criteria", name: "Must not appear", rank: 1 }],
    });
    await createStageResult({
      issueId: otherIssue._id,
      rankedAlternatives: [{ alternativeId: "other", name: "Other result", rank: 1 }],
    });
    ...
    expect(byId.get(String(consensusIssue._id)).topAlternatives).toEqual([
      { alternativeId: "final", name: "Final round", rank: 1 },
    ]);
    expect(byId.get(String(otherIssue._id)).topAlternatives).toEqual([
      { alternativeId: "other", name: "Other result", rank: 1 },
    ]);
  });
  ...
  it("handles short, missing, and malformed legacy rankings without failing the list", async () => {
    ...
    await createStageResult({
      ...
      rankedAlternatives: [{ alternativeId: "only", name: "Only alternative", rank: 1 }],
    });
    await createStageResult({
      issueId: malformedRanking._id,
      rankedAlternatives: [
        { alternativeId: "missing-name", rank: 1 },
        { name: "Missing identifier", rank: 2 },
        { alternativeId: "invalid-rank", name: "Invalid rank", rank: "none" },
        { alternativeId: "usable", name: "Usable alternative", rank: 3 },
      ],
    });
    ...
    expect(byId.get(String(shortRanking._id))).toEqual([
      { alternativeId: "only", name: "Only alternative", rank: 1 },
    ]);
