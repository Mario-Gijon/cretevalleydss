...
export const getFinishedIssueGraphAvailability = (viewIssue) => {
  ...
  return {
    hasPerformanceMap:
      (Array.isArray(directScatter) && directScatter.length > 0) ||
      Boolean(normalizedPlots?.isValid),
    hasConsensusEvolution:
      Array.isArray(consensusPoints) &&
      consensusPoints.filter((point) => Number.isFinite(Number(point))).length > 1,
    normalizedPlots,
  };
};
