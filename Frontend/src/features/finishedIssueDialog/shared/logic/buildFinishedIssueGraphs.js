...
export const normalizePlotsGraphic = (plotsGraphic) => {
  ...
  if (pointsByEmailRaw) {
    for (const [label, point] of Object.entries(pointsByEmailRaw)) {
      if (isFinitePoint(point)) expertPoints.push({ label: String(label), ...normalizePoint(point) });
    }
    ...
    expertPointsRaw.forEach((point, index) => {
      ...
      expertPoints.push({
        label: typeof labelCandidate === "string" && labelCandidate.trim() ? labelCandidate.trim() : `Expert ${index + 1}`,
