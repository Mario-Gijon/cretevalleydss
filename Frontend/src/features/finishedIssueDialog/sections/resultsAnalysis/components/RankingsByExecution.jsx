...
const RankingsByExecution = ({ executions }) => <Box sx={resultsPanelSx}>
  ...
  <Box sx={comparisonRankingsGridSx(executions.length)}>{executions.map((execution) => <Box key={execution.key} sx={executionRankingCardSx(execution.color)}>
    <Stack direction="row" spacing={0.8} alignItems="center"><Box sx={{ width: 10, height: 10, borderRadius: "50%", bgcolor: execution.color, flex: "0 0 auto" }} /><Typography noWrap title={execution.fullLabel} sx={{ minWidth: 0, fontSize: 13.5, fontWeight: 950 }}>{execution.displayLabel}</Typography></Stack>
