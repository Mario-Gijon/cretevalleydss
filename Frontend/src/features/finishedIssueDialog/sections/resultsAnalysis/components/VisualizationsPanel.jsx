...
const unavailableMessage = (reason) => {
  if (reason === "insufficient_variation_for_projection") {
    return "Analytical graph is unavailable because all expert inputs are equivalent.";
  }
  if (reason === "insufficient_points_for_projection") {
    return "Analytical graph is unavailable because there are not enough points to project.";
  }
  if (reason === "projection_failed") {
    return "Analytical graph projection failed for this result.";
  }
...
const VisualizationsPanel = ({ visualizations = {}, scatterPlotRef, onResetZoom }) => (
  <SectionCard
    ...
    right={
      visualizations.hasPerformanceMap ? (
        <Button
          variant="outlined"
          color="secondary"
          size="small"
    ...
    {visualizations.hasPerformanceMap ? (
      <Box sx={{ width: "100%", height: { xs: 290, md: 520 } }}>
        <AnalyticalScatterChart
          data={visualizations.performanceMapData}
          phase={visualizations.selectedPhase ?? 0}
      ...
      <Stack justifyContent="center" alignItems="center">
        <Typography variant="body2" color="text.secondary">
          {unavailableMessage(visualizations.unavailableReason)}
        </Typography>
      </Stack>
    )}
  </SectionCard>
