 ...
 */
export const buildCanonicalAnalyticalProjection = ({ execution, slotIndex = 0 }) => {
  const normalized = normalizePlotsGraphic(execution?.standardizedOutput?.plotsGraphic);
  const base = {
    key: execution?.key || "",
  ...
  if (!expertPoints.every((point) => point.identity && finite(point.x) && finite(point.y)) || !finite(collectivePoint.x) || !finite(collectivePoint.y)) {
    return { ...base, displayAvailable: true, expertPoints, collectivePoint: pointCopy(collectivePoint), unavailableReason: "missing_stable_expert_identity" };
  }
  expertPoints.sort(byIdentity);
  if (new Set(expertPoints.map((point) => point.identity)).size !== expertPoints.length) {
    return { ...base, displayAvailable: true, expertPoints, collectivePoint: pointCopy(collectivePoint), unavailableReason: "duplicate_expert_identity" };
  }
  return { ...base, available: true, displayAvailable: true, unavailableReason: null, expertPoints, collectivePoint: pointCopy(collectivePoint) };
};

const sameIdentitySet = (left, right) =>
  left.expertPoints.length === right.expertPoints.length &&
  left.expertPoints.every((point, index) => point.identity === right.expertPoints[index]?.identity);

export const areAnalyticalProjectionsEquivalent = (left, right, tolerance = ANALYTICAL_PROJECTION_TOLERANCE) => {
  if (!left?.available || !right?.available || !left.collectivePoint || !right.collectivePoint || !sameIdentitySet(left, right)) return false;
  return left.expertPoints.every((point, index) => samePoint(point, right.expertPoints[index], tolerance))
...
const subtract = (point, center) => ({ x: point.x - center.x, y: point.y - center.y });
...
export const alignProjectionToReference = ({ reference, candidate }) => {
  if (!reference?.available || !candidate?.available || !sameIdentitySet(reference, candidate)) {
    return { available: false, unavailableReason: "expert_identities_do_not_match" };
  }
  if (reference.expertPoints.length < 2) return { available: false, unavailableReason: "insufficient_alignment_anchors" };
  const refCenter = centroid(reference.expertPoints);
  const candidateCenter = centroid(candidate.expertPoints);
  const referenceAnchors = reference.expertPoints.map((point) => subtract(point, refCenter));
  const candidateAnchors = candidate.expertPoints.map((point) => subtract(point, candidateCenter));
  const candidateVariance = candidateAnchors.reduce((sum, point) => sum + point.x ** 2 + point.y ** 2, 0);
  const referenceVariance = referenceAnchors.reduce((sum, point) => sum + point.x ** 2 + point.y ** 2, 0);
  if (candidateVariance <= 1e-12 || referenceVariance <= 1e-12) return { available: false, unavailableReason: "degenerate_alignment_anchors" };
  ...
  const fit = (reflected) => {
    ...
    const transform = (point) => {
      ...
      return { x: refCenter.x + scale * (cos * x - sin * centered.y), y: refCenter.y + scale * (sin * x + cos * centered.y) };
    };
    const alignedAnchors = candidate.expertPoints.map(transform);
    const rmsResidual = Math.sqrt(alignedAnchors.reduce((sum, point, index) => sum + (point.x - reference.expertPoints[index].x) ** 2 + (point.y - reference.expertPoints[index].y) ** 2, 0) / alignedAnchors.length);
    return { transform, scale, reflected, rmsResidual };
  };
  const possibilities = [fit(false), fit(true)].filter(Boolean).sort((left, right) => left.rmsResidual - right.rmsResidual || Number(left.reflected) - Number(right.reflected));
  const best = possibilities[0];
  if (!best) return { available: false, unavailableReason: "invalid_alignment_transform" };
  const alignedProjection = {
    ...candidate,
    expertPoints: candidate.expertPoints.map((point) => ({ ...point, ...best.transform(point) })),
    collectivePoint: { ...candidate.collectivePoint, ...best.transform(candidate.collectivePoint) },
  };
  if (![...alignedProjection.expertPoints, alignedProjection.collectivePoint].every((point) => finite(point.x) && finite(point.y)) || !finite(best.rmsResidual)) return { available: false, unavailableReason: "invalid_alignment_transform" };
  return { available: true, unavailableReason: null, alignedProjection, diagnostics: { rmsResidual: best.rmsResidual, scale: best.scale, reflected: best.reflected } };
};
...
const groupEquivalentProjections = (projections) => projections.reduce((groups, projection) => {
  const group = groups.find((entry) => areAnalyticalProjectionsEquivalent(entry.projection, projection));
  if (group) {
    ...
    groups.push({ id: `projection-group-${groups.length + 1}`, projection, representative: projection, executions: [projection] });
  }
  return groups;
}, []);

const equalityMessage = (group) => group.executions.length > 1
...
export const buildResultsVisualizationsData = ({ payload, executions }) => {
  ...
  if (!valid.length) {
    const displayable = projections.filter((projection) => projection.displayAvailable);
    if (displayable.length) return { mode: "comparison", consensus, expertCollectiveComparison: { available: true, presentation: "separate", groups: displayable.map((projection, index) => visualGroup({ id: `projection-group-${index + 1}`, projection, representative: projection, executions: [projection] })), alignedExecutions: [], separateExecutions: displayable.map((projection, index) => visualGroup({ id: `projection-group-${index + 1}`, projection, representative: projection, executions: [projection] })), unavailableExecutions: projections.filter((projection) => !projection.available), footerMessages: ["The different stored projections could not be aligned safely, so each unique projection is shown separately."], footerMessage: "The different stored projections could not be aligned safely, so each unique projection is shown separately.", referenceGroupId: null } };
    return { mode: "comparison", consensus, expertCollectiveComparison: { available: false, presentation: "unavailable", groups: [], alignedExecutions: [], separateExecutions: [], unavailableExecutions: projections, footerMessages: ["No stored analytical projections are available for the selected executions."], footerMessage: "No stored analytical projections are available for the selected executions.", referenceGroupId: null } };
  }
  ...
  for (const group of groups) {
    if (group.id === referenceGroup.id) alignedGroups.push(visualGroup(group));
      ...
      const alignment = alignProjectionToReference({ reference, candidate: group.representative });
  ...
  if (!alignmentFailed && valid.length === projections.length) {
    const footerMessages = [...alignedGroups.map((group) => group.equalityMessage).filter(Boolean), "Different projections are aligned to the first selected projection for visual comparison."];
    return { mode: "comparison", consensus, expertCollectiveComparison: { available: true, presentation: "aligned-overlay", groups: alignedGroups, alignedExecutions: alignedGroups, separateExecutions: [], unavailableExecutions: [], footerMessages, footerMessage: footerMessages.at(-1), referenceGroupId: referenceGroup.id } };
  }
  const allGroups = [
    ...groupEquivalentProjections(valid).map((group) => visualGroup(group)),
    ...projections.filter((projection) => !projection.available && projection.displayAvailable).map((projection, index) => visualGroup({ id: `unavailable-projection-${index + 1}`, projection, representative: projection, executions: [projection] })),
  ];
  const footerMessages = ["The different stored projections could not be aligned safely, so each unique projection is shown separately."];
  return { mode: "comparison", consensus, expertCollectiveComparison: { available: allGroups.length > 0, presentation: "separate", groups: allGroups, alignedExecutions: [], separateExecutions: allGroups, unavailableExecutions: projections.filter((projection) => !projection.available), footerMessages, footerMessage: footerMessages[0], referenceGroupId: referenceGroup.id } };
};
