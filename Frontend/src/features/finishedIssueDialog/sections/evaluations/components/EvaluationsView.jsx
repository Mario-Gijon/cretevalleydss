...
const EvaluationsView = ({ data, state, actions }) => {
  ...
  return <SectionCard title="Evaluations" icon={<AnalyticsIcon fontSize="small" />}><Stack spacing={2}>
    <Stack direction={{ xs: "column", sm: "row" }} spacing={1.25}>
      ...
      {data.canShowCollective ? <ToggleButton selected={state.showCollective} onChange={() => actions.setShowCollective(!state.showCollective)} color="secondary">{state.showCollective ? "Hide collective" : "Show collective"}</ToggleButton> : null}
    </Stack>
