...
const DEFAULT_ORDERED_LABELS = Object.freeze([
  { label: "Low" },
  { label: "Medium" },
  { label: "High" },
]);
...
export const OrderedLinguisticCreationForm = ({
  value,
  onChange,
  disabled = false,
  typeKey,
  ...
  const labels = buildDraftLabels(rawLabels);
  const draftLabels = labels.length > 0
    ? labels
    : buildDraftLabels(DEFAULT_ORDERED_LABELS);
  ...
  const updateLabels = (nextLabels, nextName = value?.name) => {
    ...
    onChange?.({
      ...value,
      name: normalizeDraftName(nextName),
      typeKey,
      definition: {
        ...definition,
        labels: buildDraftLabels(nextLabels),
      },
    });
  };
  ...
  return (
    <Stack spacing={2}>
      <TextField
        ...
        onChange={(event) => updateLabels(draftLabels, event.target.value)}
        disabled={disabled}
        fullWidth
      />
      ...
      <TransitionGroup component={List} disablePadding>
        {draftLabels.map((labelItem, index) => (
              ...
              <Stack direction="row" spacing={0.75} alignItems="center" sx={{ flex: 1, minWidth: 0 }}>
                <Box sx={{ flex: 1, minWidth: 0 }}>
                  <TextField
                    ...
                    onChange={(event) => {
                      const nextLabels = draftLabels.map((item, itemIndex) =>
                        itemIndex === index
                ...
                <IconButton
                  onClick={() =>
                    updateLabels(draftLabels.filter((_, itemIndex) => itemIndex !== index))
                  }
                  disabled={disabled || draftLabels.length <= minimumLabelCount}
                  color="error"
                  size="small"
                  sx={{ flexShrink: 0 }}
                >
      ...
      <Button
        ...
        onClick={() =>
          updateLabels([...draftLabels, { label: `Label ${draftLabels.length + 1}` }])
        }
        disabled={disabled}
        size="small"
...
export default OrderedLinguisticCreationForm;
