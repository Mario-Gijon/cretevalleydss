...
export const reconcileNumberCriterionMap = ({ rows, value }) =>
  buildNumberCriterionDraft({ rows, value });

export const numberCriterionMapsEqual = (left, right) =>
  isPlainObject(left) &&
  isPlainObject(right) &&
  Object.keys(left).length === Object.keys(right).length &&
  Object.keys(left).every((key) => Object.prototype.hasOwnProperty.call(right, key) && left[key] === right[key]);
