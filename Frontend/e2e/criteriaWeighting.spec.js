...
test("criteria weighting smoke: an invited expert can submit manual criteria weights", async ({ page }) => {
  const issueName = `E2E Criteria Weighting ${Date.now()}`;

  await loginAsAuthSmokeUser(page);
  await page.getByRole("tab", { name: "Create" }).click();
  await expect(page.getByRole("heading", { name: "Create issue" })).toBeVisible();

  await page.getByText("E2E Weighted Matrix Model", { exact: true }).click();
  await expect(page.getByText("Selected", { exact: true })).toBeVisible();
  await page.getByRole("button", { name: "Next" }).click();

  await page.getByPlaceholder("Alternative").fill("E2E Weight Alpha");
  await page.getByRole("button", { name: "Add", exact: true }).click();
  await page.getByPlaceholder("Alternative").fill("E2E Weight Beta");
  await page.getByRole("button", { name: "Add", exact: true }).click();
  await page.getByRole("button", { name: "Next" }).click();

  await page.getByPlaceholder("Criterion").fill("E2E Weight Cost");
  await page.getByRole("button", { name: "Add", exact: true }).click();
  await page.getByPlaceholder("Criterion").fill("E2E Weight Quality");
  await page.getByRole("button", { name: "Add", exact: true }).click();
  ...
  await page
    ...
    .fill("Created for the Playwright criteria-weighting flow.");
  ...
  await page
    .getByText("E2E Weight Cost", { exact: true })
    .locator("..")
    .getByRole("spinbutton")
    .fill("0.6");
  await page
    .getByText("E2E Weight Quality", { exact: true })
    .locator("..")
    .getByRole("spinbutton")
    .fill("0.4");
  ...
  await expect(issueHeading).toBeVisible();
  await issueHeading.click();
  await expect(page.getByRole("button", { name: "Evaluate weights" })).toBeDisabled();
});
