...
describe("model parameter value state", () => {
  ...
  it("preserves owned values unchanged and only prunes whole parameter keys", () => {
    ...
    const previous = { scalar: -0.5, pluginMap: stalePluginMap, unknown: { arbitrary: true } };
    ...
    expect(next).toEqual({ scalar: -0.5, pluginMap: stalePluginMap, newValue: [0, 1], unknown: { arbitrary: true } });
