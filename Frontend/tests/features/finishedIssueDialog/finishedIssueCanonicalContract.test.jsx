...
import { buildResultsAnalysisData } from "../../../src/features/finishedIssueDialog/shared/logic/buildFinishedIssueResultsAnalysisData.js";
...
describe("Finished Issue canonical contract", () => {
  ...
  it("loads once, does not issue scenario reads, and refreshes after scenario writes", async () => {
    ...
    act(() => result.current.header.handleSelectRun("scenario-ok"));
    expect(result.current.runs.selectedExecution.type).toBe("scenario");
    expect(result.current.header.showRounds).toBe(false);
    act(() => result.current.header.handleSelectRun("base"));
    expect(result.current.header.selectedPhase).toBe(5);
    act(() => result.current.modelsSection.addDialog.setSelectedModelId("model-scenario"));
    await act(async () => { await result.current.modelsSection.addDialog.handleAddModelRun(); });
    ...
    act(() => result.current.header.handleSelectRun("scenario-ok"));
    await act(async () => { await result.current.modelsSection.handleRemoveSelectedRun(); });
