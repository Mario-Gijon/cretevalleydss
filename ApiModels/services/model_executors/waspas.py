...
from models.waspas.waspas_model import run_waspas
...
def _weights(payload: GenericModelExecutionRequest, criteria_count: int) -> list[float]:
    ...
    if len(raw_weights) == 0:
        raise ValueError("weights are required for WASPAS")
...
def _lambda_parameter(payload: GenericModelExecutionRequest) -> float:
    model_parameters = payload.modelParameters or {}
    raw_lambda = model_parameters.get("lambda", 0.5)
    lambda_value = _finite_number(raw_lambda, "modelParameters.lambda")

    if lambda_value < 0 or lambda_value > 1:
        raise ValueError("modelParameters.lambda must be between 0 and 1")
...
def _output(
    *,
    ...
    if not isinstance(ranking_indexes, list):
        raise ValueError("WASPAS output is missing collective_ranking")
    if not isinstance(collective_scores, list):
        raise ValueError("WASPAS output is missing collective_scores")

    ranking = [alternative_names[int(index)] for index in ranking_indexes]
    if len(ranking) == 0:
        raise ValueError("WASPAS output collective_ranking is empty")
...
def execute_waspas(payload: GenericModelExecutionRequest) -> dict[str, Any] | JSONResponse:
    try:
        ...
        return error_response(f"Error executing WASPAS: {error}", code="INTERNAL_ERROR")