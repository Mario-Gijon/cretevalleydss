...
from issue_scenario_lab.scenarios.no_consensus_basic import SCENARIO_ID, generate
...
class FakeClient:
    ...
    def request(self, method: str, path: str, *, json: Any = None) -> Any:
                ...
                "models": [
                    {
                        ...
                        "supportedExpressionDomains": ["numericDiscrete"],
                    }
                ]
            }
        if self.alias == "owner" and path == "/issues/users":
            return {
                "users": [
                    {"email": "a@example.test", "confirmed": True, "selectable": True},
                    {"email": "b@example.test", "confirmed": True, "selectable": True},
                ]
            }
        if self.alias == "owner" and path == "/issues/expression-domains":
            return {"globalDomains": [{"id": "domain-id", "scope": "global", "typeKey": "numericDiscrete", "min": 1, "max": 5, "step": 1}]}
            ...
            return {
                ...
                "payload": {
                    "context": {
                        ...
                        "criteria": [
                            {"id": "quality", "name": "Quality", "expressionDomain": {"typeKey": "numericDiscrete", "min": 1, "max": 5, "step": 1}},
                            {"id": "cost", "name": "Cost", "expressionDomain": {"typeKey": "numericDiscrete", "min": 1, "max": 5, "step": 1}},
                        ],
                    }
                },
            }
        ...
        if path.endswith("/compute"):
            return {"currentStage": "finished", "result": {"rankedAlternatives": [{}, {}, {}]}}
        if path == "/issues/finished":
            return {"issues": [{"id": "issue-id"}]}
        if path == "/issues/finished/issue-id":
            return {"id": "issue-id", "isConsensus": False}
        return {}
...
def test_numeric_levels_are_valid_for_continuous_and_discrete_domains() -> None:
    assert numeric_levels({"typeKey": "numericContinuous", "min": 0, "max": 8}) == (2.0, 4.0, 6.0)
    assert numeric_levels({"typeKey": "numericDiscrete", "min": 0.1, "max": 0.3, "step": 0.1}) == (0.1, 0.2, 0.3)
    assert numeric_levels({"typeKey": "numericDiscrete", "min": 1, "max": 2, "step": 1}) == (1.0, 2.0, 2.0)
...
def test_complete_mocked_flow_writes_one_minimal_manifest_entry(tmp_path: Path) -> None:
    ...
    assert submitted[0][2]["payload"]["balanced"]["quality"] == {"value": 3.0}
