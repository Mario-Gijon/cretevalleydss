...
def load_fixture(path: Path = FIXTURE_PATH) -> dict[str, Any]:
    ...
    if (
        ...
        or any(len(row) != 18 for row in data.get("sourceValues", {}).values())
    ):
    ...
    if not isinstance(experts, list) or len(experts) != 5 or len(set(experts)) != 5:
        raise ScenarioLabError("two-tuple Greece fixture must contain five unique weighting experts")
    if participants.get("finalExpert") not in experts:
        raise ScenarioLabError("two-tuple Greece fixture finalExpert must be a criteria-weighting expert")
    ...
    if set(rankings) != set(experts) or any(
        ...
        raise ScenarioLabError("two-tuple Greece fixture rankings must cover all seven parents for every expert")
...
def _validate_finished_audit(
    ...
    final_email = sessions.users[data["participants"]["finalExpert"]].email.casefold()
    current = [item for item in participants if ((item.get("expert") or {}).get("email") or "").casefold() == final_email]
    if len(participants) != 1 or len(current) != 1 or current[0].get("invitationStatus") != "accepted":
        raise ScenarioLabError("finished issue does not have exactly one accepted final expert")
    ...
    final_id = expert_ids_by_email.get(final_email)
    if len(alternative_evaluations) != 1 or alternative_evaluations[0].get("expertId") != final_id:
        raise ScenarioLabError("finished issue does not preserve exactly one final expert evaluation")
...
def generate(sessions: SessionPool, store: ManifestStore, *, owner_alias: str = "owner", fixture_path: Path = FIXTURE_PATH) -> Any:
    ...
    try:
        ...
        owner.edit_experts(
            issue_id,
            experts_to_add=[],
            experts_to_remove=[sessions.users[alias].email for alias in experts if alias != expert_alias],
            expert_weights_by_email={sessions.users[expert_alias].email: 1.0},
        )
        active = _active_issue(owner, issue_id)
        current_participants = _items(active, "participants")
        if current_participants:
            active_emails = {((item.get("expert") or {}).get("email") or "").casefold() for item in current_participants}
            if active_emails != {sessions.users[expert_alias].email.casefold()}:
                raise ScenarioLabError("expert removal did not leave only the final expert")
        context = _context(expert.evaluation(issue_id, ALTERNATIVE_STAGE), issue_id, ALTERNATIVE_STAGE, MODEL_KEY)
        alternatives, criterion_ids = _leaf_context_maps(data, context, expected_leaf)
        expert.submit_evaluation(issue_id, ALTERNATIVE_STAGE, _matrix(data, context, alternatives=alternatives, criteria=criterion_ids))
        ...
        entry = GeneratedIssue(
            ...
            visibleUserAliases=[owner_alias, expert_alias],
        )
        store.add(entry)
        return type(
            "GenerationResult",
