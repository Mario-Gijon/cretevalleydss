...
def _v_parameter(payload: GenericModelExecutionRequest) -> float:
    model_parameters = payload.modelParameters or {}
    raw_v = model_parameters.get("v", 0.5)
    v = _finite_number(raw_v, "modelParameters.v")

    if v < 0 or v > 1:
        raise ValueError("modelParameters.v must be between 0 and 1")
...
def _output(
    *,
    ...
    if not isinstance(ranking_indexes, list):
        raise ValueError("VIKOR output is missing collective_ranking")
    if not isinstance(collective_scores, list):
        raise ValueError("VIKOR output is missing collective_scores")

    if len(ranking_indexes) == 0:
        raise ValueError("VIKOR output collective_ranking is empty")
    ...
    for rank_position, ranking_index in enumerate(ranking_indexes, start=1):
        ...
        if alternative_index < 0 or alternative_index >= len(alternative_names):
            raise ValueError("VIKOR collective_ranking contains out-of-range index")
...
def execute_vikor(payload: GenericModelExecutionRequest) -> dict[str, Any] | JSONResponse:
    try:
        ...
        return error_response(f"Error executing VIKOR: {error}", code="INTERNAL_ERROR")
