...
def _output(
    *,
    ...
    if not isinstance(ranking_indexes, list):
        raise ValueError("MARCOS output is missing collective_ranking")
    if not isinstance(collective_scores, list):
        raise ValueError("MARCOS output is missing collective_scores")

    if len(ranking_indexes) == 0:
        raise ValueError("MARCOS output collective_ranking is empty")
    ...
    for rank_position, ranking_index in enumerate(ranking_indexes, start=1):
        ...
        if alternative_index < 0 or alternative_index >= len(alternative_names):
            raise ValueError("MARCOS collective_ranking contains out-of-range index")
...
def execute_marcos(payload: GenericModelExecutionRequest) -> dict[str, Any] | JSONResponse:
    try:
        ...
        return error_response(f"Error executing MARCOS: {error}", code="INTERNAL_ERROR")
